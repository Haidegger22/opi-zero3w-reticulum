...
## Имя узла и определение отправителя

Мост задаёт модели, кто она, и сообщает, кто пишет. Без этого копия скрипта на другом узле
начинает выдавать себя за первый узел, а агент путает себя с собеседником.

- `Environment=LXMF_SELF=<кто я>` — описание узла, попадает в системный промпт моста.
  Пример: `LXMF_SELF=Джарвис — Hermes на Raspberry Pi 5 (узел 192.168.1.81)`.
- `Environment=LXMF_BOT_NAME=<имя в сети>` — имя, под которым узел виден в LXMF.
- Отправитель определяется по адресу и подставляется в начало сообщения:
  `[от Артёма (смартфон)]`, `[от Джарвиса (Hermes на Raspberry Pi 5)]`,
  `[от Зеро (Hermes на Orange Pi Zero 3W)]`, `[автотест проверки связи]`.
  Таблица адресов — в начале `scripts/hermes_bridge.py` (`SENDERS`); дополните её своими.

Проверка, что всё в порядке: спросить мост «представься одной строкой» — он должен назвать
себя и свою машину.
...
5. Если узлов несколько, все мосты удобнее направить на **один общий накопитель** (например на
   постоянно включённой машине). Тогда в приложении достаточно выбрать один этот узел и не искать,
   где именно лежит сообщение.
6. В приложении на телефоне: **Настройки → Connection → раздел Propagation → Propagation node**.
